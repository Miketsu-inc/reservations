...
func (s *Service) NewType(ctx context.Context, input NewTypeInput) error {
	...
	err := s.blockedTimeRepo.NewBlockedTimeType(ctx, employee.MerchantId, domain.BlockedTimeType{
		Id:       0,
		Name:     input.Name,
...
func (s *Service) UpdateType(ctx context.Context, blockedTimeTypeId int, input UpdateTypeInput) error {
	...
	err := s.blockedTimeRepo.UpdateBlockedTimeType(ctx, employee.MerchantId, domain.BlockedTimeType{
		Id:       input.Id,
		Name:     input.Name,
		Duration: input.Duration,
		Icon:     input.Icon,
...
func (s *Service) DeleteType(ctx context.Context, blockedTimeTypeId int) error {
	...
	err := s.blockedTimeRepo.DeleteBlockedTimeType(ctx, employee.MerchantId, blockedTimeTypeId)
	if err != nil {
...
func (s *Service) GetTypes(ctx context.Context) ([]domain.BlockedTimeType, error) {
	...
	blockedTimetypes, err := s.blockedTimeRepo.GetAllBlockedTimeTypes(ctx, employee.MerchantId)
	if err != nil {
