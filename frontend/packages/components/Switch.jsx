...
export default function Switch({
  ...
  return (
    <button
      ...
      className={`${
        isOn
          ? `${variant === "monochrome" ? "bg-black dark:bg-white" : "bg-primary"}`
    ...
    >
      <div
        className={`${isOn ? `${size === "small" ? "translate-x-3" : size === "medium" ? "translate-x-4" : size === "large" ? "translate-x-5" : ""}` : "translate-x-0"}
          ...
          pointer-events-none rounded-full bg-white transition-transform
          duration-100 outline-none dark:bg-black`}
      ></div>
    </button>
  );
}
